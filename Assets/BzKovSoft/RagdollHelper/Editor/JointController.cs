...
namespace BzKovSoft.RagdollHelper.Editor
{
	...
	public static class JointController
	{
		...
		static float ProcessLimit(Vector3 position, Vector3 planeNormal, Vector3 startDir, float size, float limit)
		{
			...
			var fmh_211_65_638280891126863656 = Quaternion.identity; bool positionChanged = Handles.FreeMoveHandle(controllerPos, size * 0.1f, Vector3.zero, Handles.SphereHandleCap) != controllerPos;
			if (positionChanged)
			{
